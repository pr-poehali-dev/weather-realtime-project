...
const generateAiResponse = async (messages: Message[], model: AiModel): Promise<string> => {
  ...
  if (lastUserMessage.toLowerCase().includes('привет') || lastUserMessage.toLowerCase().includes('здравствуй')) {
    response = 'Привет! Чем я могу помочь вам сегодня?';
  } 
  else if (lastUserMessage.toLowerCase().includes('погода')) {
    response = 'Я не могу проверить текущую погоду в реальном времени, но могу помочь вам найти источники прогноза погоды или ответить на общие вопросы о климате.';
  }
  else if (lastUserMessage.toLowerCase().includes('кто ты') || lastUserMessage.toLowerCase().includes('как тебя зовут')) {
    ...
    response = 'Всегда рад помочь! Если у вас возникнут еще вопросы, обращайтесь.';
  }
  else if (lastUserMessage.toLowerCase().includes('возможности') || lastUserMessage.toLowerCase().includes('что ты умеешь')) {
    ...
    response = 'Могли бы вы уточнить свой вопрос? Я готов помочь, но мне нужно больше информации.';
  }
  else {
...
export const changeModel = (session: ChatSession, modelId: string): ChatSession => {
  const model = getModelById(modelId);
  return {
    ...session,
    model,
